              ...
              <div className="flex flex-wrap gap-2 mt-2">
                {club.club_label_assignments?.map((assignment) => (
                  <Badge
                    key={assignment.id}
                    variant="secondary"
                    className="bg-white/10 text-white pointer-events-none whitespace-nowrap"
                  >
                    {assignment.club_labels.name === 'Performance' && <Trophy className="h-3 w-3 mr-1" />}
                    {assignment.club_labels.name === 'Track' && <Route className="h-3 w-3 mr-1" />}
                    {assignment.club_labels.name === 'Road Running' && <MapPin className="h-3 w-3 mr-1" />}
                    {assignment.club_labels.name === 'Social' && <Users className="h-3 w-3 mr-1" />}
